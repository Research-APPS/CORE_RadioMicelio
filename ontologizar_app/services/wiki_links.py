...
@lru_cache(maxsize=4)
def _link_index(site_root: str) -> tuple[tuple[str, str, str], dict[str, str]]:
...
def clear_link_index_cache() -> None:
    _link_index.cache_clear()


def _resolve_bracket(key: str, display: str, site_root: str) -> str:
    ordered, explicit = _link_index(site_root)
...
def _autolink_terms(escaped_text: str, site_root: str) -> str:
    if not escaped_text:
        return ""
    ordered, _ = _link_index(site_root)
    result = escaped_text
...
def linkify_plaintext(text: str, *, site_root: str = "/") -> str:
    """Texto plano → HTML con [[wiki]] y auto-enlaces a fichas de la biblioteca."""
    ...
    for match in _WIKI_BRACKET.finditer(text):
        if match.start() > last:
            parts.append(_autolink_terms(escape(text[last:match.start()]), site_root))
        parts.append(_resolve_bracket(match.group(1), match.group(2) or "", site_root))
        last = match.end()
    if last < len(text):
        parts.append(_autolink_terms(escape(text[last:]), site_root))
    return "".join(parts)


def linkify_paragraphs(text: str, *, site_root: str = "/") -> str:
    blocks = [p.strip() for p in text.split("\n\n") if p.strip()]
    if not blocks:
        blocks = [line.strip() for line in text.split("\n") if line.strip()]
    return "".join(f"<p>{linkify_plaintext(block, site_root=site_root)}</p>" for block in blocks)
